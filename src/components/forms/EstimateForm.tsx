...
export const EstimateForm: React.FC = () => {
  ...
  const existingEstimate = id ? getEstimateById(id) : null;
  ...
  useEffect(() => {
    if (existingEstimate) {
      ...
      setLineItems(existingEstimate.lineItems);
    }
  }, [existingEstimate]);
  ...
  const handleSubmit = (status: 'draft' | 'sent' | 'accepted' | 'rejected' | 'expired') => {
